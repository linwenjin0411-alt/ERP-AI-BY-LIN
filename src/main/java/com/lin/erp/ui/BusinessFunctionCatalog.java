...
public final class BusinessFunctionCatalog {
    ...
    private static void addAdminPages() {
        add("ADMIN_USERS",
                new String[]{"column.id", "column.name", "column.role", "column.department", "column.email", "column.language", "column.status", "function.field.memo"},
                new String[]{"USR-admin", "System Administrator", "role.admin", "IT", "admin@linova.local", "Japanese", "status.open", "Login user maintenance"},
                new String[]{"column.id", "column.name", "column.role", "column.department", "column.status", "column.next"},
                new String[][]{{"USR-admin", "user.admin.name", "role.admin", "IT", "status.open", "term.roles"}, {"USR-planner", "user.planner.name", "role.planner", "Production", "status.released", "term.permissions"}},
                "User -> Role -> Menu permission",
                "Company and role",
                "Role permissions");
        add("ADMIN_ROLES",
                new String[]{"column.id", "column.name", "column.scope", "column.status", "column.owner", "function.field.memo"},
                new String[]{"ROLE-PLN", "role.planner", "Manufacturing / Inventory", "status.released", "owner.system", "Planner role"},
                new String[]{"column.id", "column.name", "column.scope", "column.status", "column.next"},
                new String[][]{{"ROLE-ADM", "role.admin", "All modules", "status.released", "term.permissions"}, {"ROLE-PLN", "role.planner", "Manufacturing / Inventory", "status.released", "term.permissions"}},
                "Role -> User / Menu permission",
                "User and organization",
                "Menu permission");
        add("ADMIN_PERMISSIONS",
                new String[]{"column.id", "column.role", "column.permission", "column.scope", "column.status", "column.owner", "function.field.memo"},
                new String[]{"PERM-PLN", "ROLE-PLN", "View / Create / Update", "PROCUREMENT", "status.open", "owner.system", "View/create/update procurement menus"},
                new String[]{"column.id", "column.role", "column.permission", "column.scope", "column.status", "column.next"},
                new String[][]{{"PERM-PO", "ROLE-PLN", "menu.procurement.po", "PROCUREMENT", "status.released", "menu.admin.audit"}, {"PERM-MO", "ROLE-PLN", "menu.manufacturing.order", "MANUFACTURING", "status.open", "menu.admin.audit"}},
